...
Use this prompt with Claude Code, Copilot, Cursor, or any other coding assistant:

````markdown
Help me set up Agent Control on my Windows machine.

Repo: https://github.com/BlguunBN/Agent-Control

What it does:
- manages Hermes in WSL
- manages OpenClaw on Windows
- includes a simple GUI for start / stop / restart / refresh

Please:
1. clone the repo if needed
2. run the setup script
3. check for missing dependencies
4. launch the app when setup is done

If anything fails, explain the fix and the exact command to run.
````
